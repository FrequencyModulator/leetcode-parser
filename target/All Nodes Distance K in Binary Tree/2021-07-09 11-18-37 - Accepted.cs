...
Runtime: 244 ms
Memory: 31.2 MB
URL: http://leetcode.com/submissions/detail/519850218/
Submission DateTime: July 9, 2021 11:18:37 AM
*/
/**
...
public class Solution {
    ...
    {
        // 1. dfs from root to target and save distance from each ONLY TRAVERSED nodes to targed
        Dictionary<TreeNode, int> distances = new Dictionary<TreeNode, int>();
        FindDistances(root, target, distances);      
        // 2, dfs from root: if node has distance (see 1) use that distance
    ...
    private void Dfs(TreeNode root, int dist, Dictionary<TreeNode, int> distances, int k, List<int> result)
    {
      ...
      // if there is distance from node to target then use it
